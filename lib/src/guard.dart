...
class JsonFieldGuard {
  ...
  static T guard<T>(String key, dynamic value, T Function(dynamic) parser) {
    try {
      ...
      throw FormatException(_buildDetailedErrorMessage<T>(key, value, e));
    }
  }
  ...
  static String _buildDetailedErrorMessage<T>(
    String key,
    dynamic value,
    Object exception,
  ) {
    ...
    buffer.writeln(
      "   😕 JSON response contains: ${_getSimpleTypeName(value.runtimeType)}",
    );
    ...
    buffer.writeln(
      "   Your code expected '$key' to be ${_getTypeExplanation<T>()},",
    );
    buffer.writeln(
      "   but the API/JSON sent ${_getValueExplanation(value)} instead.",
    );
    ...
    if (T == int && value is String) {
      ...
      buffer.writeln(
        "            int.parse('$value')  // This gives you ${int.tryParse(value) ?? 'ERROR'}",
      );
      buffer.writeln("");
      buffer.writeln("   Option 2: Use our safe method");
      buffer.writeln(
        "            json.getSafe('$key', (v) => int.parse(v as String))",
      );
      buffer.writeln("");
      buffer.writeln(
        "   Option 3: Ask your backend team to send a number instead of '$value'",
      );
      buffer.writeln("");
      ...
      buffer.writeln(
        "       $key: json.getSafe('$key', (v) => int.parse(v as String)),",
      );
      buffer.writeln("");
      buffer.writeln("   ✂️  @JsonKey annotation approach:");
      buffer.writeln("       @JsonKey(fromJson: _stringToInt) final int $key;");
      buffer.writeln(
        "       static int _stringToInt(dynamic value) => int.parse(value as String);",
      );
    } else if (T == double && value is String) {
      buffer.writeln("🔧 How to fix this (3 easy options):");
      buffer.writeln("   Option 1: Convert the string to a decimal number");
      buffer.writeln(
        "            double.parse('$value')  // This gives you ${double.tryParse(value) ?? 'ERROR'}",
      );
      buffer.writeln("");
      buffer.writeln("   Option 2: Use our safe method");
      buffer.writeln(
        "            json.getSafe('$key', (v) => double.parse(v as String))",
      );
      buffer.writeln("");
      buffer.writeln(
        "   Option 3: Ask your backend team to send a decimal number instead of '$value'",
      );
      buffer.writeln("");
      buffer.writeln("📋 COPY-PASTE READY SOLUTIONS:");
      buffer.writeln("   ✂️  Quick fix in fromJson method:");
      ...
      buffer.writeln("   ✂️  Safe parsing method:");
      buffer.writeln(
        "       $key: json.getSafe('$key', (v) => double.parse(v as String)),",
      );
      buffer.writeln("");
      buffer.writeln("   ✂️  @JsonKey annotation approach:");
      buffer.writeln(
        "       @JsonKey(fromJson: _stringToDouble) final double $key;",
      );
      buffer.writeln(
        "       static double _stringToDouble(dynamic value) => double.parse(value as String);",
      );
    } else if (T == bool && value is int) {
      buffer.writeln("🔧 How to fix this (3 easy options):");
      buffer.writeln("   Option 1: Convert the number to true/false");
      buffer.writeln(
        "            $value == 1  // This gives you ${value == 1}",
      );
      buffer.writeln("            (1 = true, 0 = false)");
      buffer.writeln("");
      buffer.writeln("   Option 2: Use our safe method");
      buffer.writeln(
        "            json.getSafeBool('$key')  // This handles 0/1 automatically!",
      );
      buffer.writeln("");
      buffer.writeln(
        "   Option 3: Ask your backend team to send true/false instead of $value",
      );
      buffer.writeln("");
      buffer.writeln("📋 COPY-PASTE READY SOLUTIONS:");
      buffer.writeln("   ✂️  Quick fix in fromJson method:");
      ...
      buffer.writeln(
        "       $key: json.getSafeBool('$key'),  // Handles 0/1, 'true'/'false', etc.",
      );
      buffer.writeln("");
      buffer.writeln("   ✂️  @JsonKey annotation approach:");
      buffer.writeln("       @JsonKey(fromJson: _intToBool) final bool $key;");
      buffer.writeln(
        "       static bool _intToBool(dynamic value) => (value as int) == 1;",
      );
      ...
      buffer.writeln("   Option 2: Use our safe method");
      buffer.writeln(
        "            json.getSafeBool('$key')  // This handles 'true'/'false' automatically!",
      );
      buffer.writeln("");
      buffer.writeln(
        "   Option 3: Ask your backend team to send true/false instead of '$value'",
      ...
      buffer.writeln("   Option 2: Use our safe method");
      buffer.writeln(
        "            json.getSafeDateTime('$key')  // This handles different date formats!",
      );
    } else {
      buffer.writeln("🔧 How to fix this:");
      buffer.writeln(
        "   1. Check if '$key' should really be ${_getSimpleTypeName<T>()}",
      );
      buffer.writeln("   2. Or write a custom converter:");
      buffer.writeln(
        "      json.getSafe('$key', (v) => /* your conversion code */)",
      );
    ...
    buffer.writeln(
      "   APIs sometimes send numbers as text, or use 0/1 instead of true/false.",
    );
    buffer.writeln(
      "   This is totally normal - you just need to convert them!",
    );
  ...
  static String _buildEnhancedErrorMessage<T>(
    ...
    buffer.writeln(
      "❌ Error parsing key '$key'${fieldDescription != null ? ' ($fieldDescription)' : ''}:",
    );
