...
extension SafeJsonExtension on Map<String, dynamic> {
  ...
  String? getNullableSafeString(String key) => getNullableSafe(key, (v) => v as String);
  ...
  int? getNullableSafeInt(String key) => getNullableSafe(key, (v) => (v as num).toInt());
  ...
  /// ```
  double getSafeDouble(String key) => getSafe(key, (v) => (v as num).toDouble());
  double? getNullableSafeDouble(String key) => getNullableSafe(key, (v) => (v as num).toDouble());
  ...
  DateTime? getNullableSafeDateTime(String key) => getNullableSafe(key, _parseDateTime);
  ...
  T? getNullableSafeObject<T>(String key, T Function(Map<String, dynamic>) factory) {
  ...
  /// ```
  List<T> getSafeObjectList<T>(String key, T Function(Map<String, dynamic>) factory) {
    return getSafeList(key, (v) => factory(v as Map<String, dynamic>));
  }

  List<T>? getNullableSafeObjectList<T>(String key, T Function(Map<String, dynamic>) factory) {
  ...
  String analyzePropertyMapping(List<String> expectedProperties) {
    ...
    final missing = expectedProperties.where((prop) => !containsKey(prop)).toList();
    final extra = jsonKeys.where((key) => !expectedProperties.contains(key)).toList();
    final matching = expectedProperties.where((prop) => containsKey(prop)).toList();
    ...
    if (missing.isNotEmpty) {
      ...
      for (final prop in missing) {
        buffer.writeln("   ✗ '$prop': Expected by model but NOT in JSON response");
    ...
    if (extra.isNotEmpty) {
      ...
      buffer.writeln("");
      buffer.writeln("   💡 These extra properties won't cause errors but might indicate:");
    ...
    buffer.writeln("   ✅ Matching: ${matching.length}/${expectedProperties.length} properties");
    buffer.writeln("   ❌ Missing: ${missing.length} properties");
    buffer.writeln("   ➕ Extra: ${extra.length} properties");
    
    if (missing.isEmpty && extra.isEmpty) {
      buffer.writeln("   🎉 Perfect match! JSON response matches your model exactly.");
    } else if (missing.isEmpty) {
      buffer.writeln("   ⚠️  JSON has extra data but no missing required fields.");
    } else {
      buffer.writeln("   🚨 Action needed: Missing properties may cause parsing errors.");
    }
  ...
  String generateModel(String className, {List<String>? expectedFields}) {
    ...
    buffer.writeln("// Copy-paste this into your project and customize as needed");
    buffer.writeln("");
    buffer.writeln("import 'package:json_annotation/json_annotation.dart';");
    buffer.writeln("import 'package:json_annotation_tools/json_annotation_tools.dart';");
    buffer.writeln("");
    ...
    buffer.writeln("  factory $className.fromJsonSafe(Map<String, dynamic> json) {");
    ...
    for (final key in jsonKeys) {
      ...
      if (safeMethod.isNotEmpty) {
        ...
        buffer.writeln("      $fieldName: json.getSafe('$key', (v) => v as ${_getDartType(value)}),");
      }
    }
    buffer.writeln("    );");
    buffer.writeln("  }");
    ...
    buffer.write("    return '$className(${jsonKeys.map((key) => '${_toCamelCase(key)}: \$${_toCamelCase(key)}').join(', ')})';");
    buffer.writeln("");
    buffer.writeln("  }");
    ...
    buffer.writeln("// try {");
    buffer.writeln("//   final user = $className.fromJsonSafe(apiResponse.data);");
    buffer.writeln("//   print('Success: \$user');");
    buffer.writeln("// } catch (e) {");
    buffer.writeln("//   print('JSON Error: \$e');  // Get detailed error message");
    buffer.writeln("// }");
  ...
  String _buildMissingKeyError(String missingKey, List<String> availableKeys) {
    ...
    buffer.writeln("   1. Check your spelling - is '$missingKey' spelled correctly?");
    buffer.writeln("   2. Look at the available fields above");
    buffer.writeln("   3. Use the exact same spelling and capitalization");
    buffer.writeln("   4. Or use getNullableSafe() if this field might not always exist");
    
    // Add copy-paste solutions based on mismatch analysis
    final copyPasteAnalysis = _analyzePropertyMismatch(missingKey, availableKeys);
    if (copyPasteAnalysis.contains("Case sensitivity issue")) {
      final exactMatch = availableKeys.firstWhere((key) => key.toLowerCase() == missingKey.toLowerCase());
      buffer.writeln("");
      buffer.writeln("📋 COPY-PASTE READY SOLUTIONS:");
      buffer.writeln("   ✂️  Fix the field name in your model:");
      buffer.writeln("       final String $exactMatch;  // Change from '$missingKey'");
      buffer.writeln("");
      buffer.writeln("   ✂️  Or use @JsonKey annotation:");
      buffer.writeln("       @JsonKey(name: '$exactMatch') final String $missingKey;");
      ...
      final correctField = availableKeys.contains(camelCase) ? camelCase : 
                           availableKeys.contains(snakeCase) ? snakeCase : null;
      
      if (correctField != null) {
        buffer.writeln("");
        buffer.writeln("📋 COPY-PASTE READY SOLUTIONS:");
        buffer.writeln("   ✂️  Use @JsonKey annotation:");
        buffer.writeln("       @JsonKey(name: '$correctField') final String $missingKey;");
        buffer.writeln("");
        buffer.writeln("   ✂️  Or change your model field name:");
        buffer.writeln("       final String $correctField;  // Change from '$missingKey'");
      }
    } else {
      ...
      buffer.writeln("       $missingKey: json.getNullableSafeString('$missingKey'),");
      buffer.writeln("");
      if (similarKeys.isNotEmpty) {
        buffer.writeln("   ✂️  Use the most similar field instead:");
        buffer.writeln("       @JsonKey(name: '${similarKeys.first}') final String $missingKey;");
        buffer.writeln("");
      }
      buffer.writeln("   ✂️  Add default value:");
      buffer.writeln("       $missingKey: json.getNullableSafeString('$missingKey') ?? 'default_value',");
    }
  ...
  String _buildMissingKeysError(List<String> missingKeys, List<String> availableKeys) {
    final buffer = StringBuffer();
  ...
  String _analyzePropertyMismatch(String expectedKey, List<String> availableKeys) {
    final expectedLower = expectedKey.toLowerCase();
    final buffer = StringBuffer();
    
    // Check for exact case mismatch
    final exactCaseMatch = availableKeys.where((key) => key.toLowerCase() == expectedLower).toList();
    ...
    for (final available in availableKeys) {
      if (available == camelCaseVariant && available != expectedKey) {
        ...
        buffer.writeln("   💡 Solution: Update model or use @JsonKey annotation");
        return buffer.toString();
      }
    ...
    final partialMatches = availableKeys.where((key) => 
      key.toLowerCase().contains(expectedLower) || expectedLower.contains(key.toLowerCase())
    ).toList();
    ...
    if (partialMatches.isNotEmpty) {
      ...
      buffer.writeln("   💡 Solution: Check API documentation for exact field names");
      return buffer.toString();
    }
    ...
    buffer.writeln("   💡 Solution: Check with backend team or API documentation");
